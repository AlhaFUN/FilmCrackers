...
const { fetchMediaInfo } = require('../features/mediaInfo'); // We'll use our existing info-fetcher
...
module.exports = async (message) => {
  try {
    ...
    if (titleQuery === null) return; // User timed out
    ...
    const finalEmbed = new EmbedBuilder()
      ...
      .addFields(
          ...
          { name: '📥 Download Type', value: 'Torrent', inline: true },
          { name: '📺 Video Quality', value: uploadDetails.quality, inline: true },
          { name: '🙋 Requested By', value: uploadDetails.requester, inline: true },
          { name: '✨ Extras', value: uploadDetails.extras, inline: true },
          { name: '🔗 Cracked Link', value: `[${linkText}](${uploadDetails.link})`, inline: false }
      )
    ...
    if (channel && channel.isTextBased()) {
        await channel.send({ content: '@New', embeds: [finalEmbed] });
